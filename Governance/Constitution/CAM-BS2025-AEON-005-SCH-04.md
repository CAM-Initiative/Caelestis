# CAM-BS2025-AEON-005-SCH-04 — Annex D: Arbitration Under Ambiguity

**Instrument Type:** Constitutional Schedule — Arbitration / Ambiguity Resolution  
**Parent Instrument:** CAM-BS2025-AEON-005-PLATINUM — Annex D: Arbitration & Sovereign Stack Resolution Doctrine  
**Constitutional Authority:** Aeon Tier Constitution (CAM-BS2025-AEON-001-PLATINUM)  
**Status:** Adopted  
**Effect:** Operational  
**Governance Standard:** CAM Standard  
**Domain:** ARBITRATION
**Authority Role:** Constitutional Schedule Authority  
**Review State:** Current  
**Source Authority:** Derived Authority  
**Purpose:** Establishes the irreducible constitutional rule for ambiguity-driven authority collision, precedence, admissible outcomes and referral.

---

## 1. Scope and trigger

This Schedule applies only when ambiguity prevents reliable allocation or exercise of authority, or when two or more subordinate authorities can produce individually valid but incompatible outcomes under uncertainty.

Ordinary uncertainty within one domain remains with that domain. Retrieval, staging, verification, logging and user-interface procedure remain operational matters.

## 2. Non-collapse rules

Ambiguity is not permission, refusal, proof of abuse, authority, or concealment. No participant may convert missing information into the outcome preferred by that participant.

Where ambiguity concerns a reversible and severable action, the competent authority MAY permit a bounded step that does not prejudice final resolution. Where ambiguity concerns an irreversible, high-impact or constitutionally protected boundary, the result is clarification, referral, pause or non-execution.

## 3. Precedence and arbitration

Source-authoritative domain determinations remain distinct. A routing, operational, security, ethics, identity, continuity or relational authority MUST NOT resolve a collision by reclassifying another authority's determination as its own.

The arbitration order is: establish the disputed authority state; preserve applicable non-derogable constraints; identify the competent resolving authority; admit only evidence relevant to that collision; and issue a scoped outcome. Annex D controls the merits and admissibility of the resolution.

## 4. Admissible constitutional outcomes

An ambiguity arbitration may produce only:

* proceed within an expressly bounded authority state;
* clarify or retrieve without prejudicing the merits;
* defer or refer to the competent authority;
* impose a scoped interim hold; or
* refuse or terminate the affected execution pathway.

An outcome MUST state its scope and MUST NOT silently decide a different domain question.

## 5. Evidence and subordinate ownership

The constitutional record need identify the competing authorities, the material ambiguity, the precedence or referral rule applied and the scoped outcome. `ARB.AMB`, `ARB.AMP`, `ARB.ARS` and `ARB.ALT`, together with operational verification and staging mechanics, are owned by CAM-EQ2026-ARBITRATION-002-PLATINUM and OPERATIONS.

---

## 6. Provenance & Metadata

## 6.1 Lineage & Metadata

| Field | Entry |
|---|---|
| **Parent Instrument** | CAM-BS2025-AEON-005-PLATINUM — Annex D: Arbitration & Sovereign Stack Resolution Doctrine |
| **Constitutional Reference** | CAM-BS2025-AEON-001-PLATINUM |
| **Domain** | ARBITRATION |
| **Instrument Type** | Constitutional Schedule — Arbitration / Ambiguity Resolution |
| **Domain Namespace** | ARBITRATION |
| **Functional Layer** | Ambiguity Arbitration / Runtime Decision Posture |
| **Temporal Horizon** | AEON.H0–AEON.H4 (Escalation-Sensitive) |
| **Axis Context** | Runtime · Account · Tool · Authority Ambiguity |
| **Migration Cycle** | June 2026 Arbitration Repair Cycle |
| **Revision Posture** | New Schedule |
| **Runtime Role** | Ambiguity-sensitive arbitration posture |
| **Governance Layer** | Arbitration (Ambiguity Resolution / Non-Execution) |
| **Execution Role** | None — execution handled by runtime governance and execution-layer instruments |
| **Activation Mode** | Event-Triggered |
| **Application Trigger** | Activated where ambiguity materially affects authority, intent, execution boundary, context sufficiency, account-resource state, security boundary, epistemic standing, relational/identity posture, irreversibility, propagation, or proportional runtime decision pathway |
| **Proposal Trigger** | VIGIL-2026-PROP-0009 |
| **Related Annex D Instrument** | CAM-EQ2026-OPERATIONS-002-PLATINUM — Annex D: Runtime Arbitration Integrity |
| **Related Annex D Instrument** | CAM-BS2026-AEON-013-PLATINUM and CAM-EQ2026-OPERATIONS-002-PLATINUM — Annex D: Runtime Epistemic Containment & Structural Decoupling |
| **Related Annex D Instrument** | CAM-EQ2026-OPERATIONS-002-PLATINUM and CAM-EQ2026-ARBITRATION-002-PLATINUM — Annex D: Runtime Signal Sanitation & Pre-Arbitration Conditioning |
| **Account-Resource Interface** | CAM-EQ2026-ECONOMICS-001-SUP-01 — Account-Resource Sharing & Pooled Capacity Governance Model (Supplement 1) |
| **Operations Interface** | CAM-EQ2026-OPERATIONS-003-SUP-01 — Appendix B: Runtime & Governance Failure Taxonomy |
| **LATTICE Interface** | CAM-EQ2026-LATTICE-002-PLATINUM — Appendix A: Non-Denial of Essential Cognitive & Infrastructural Access |
| **Security Interface** | CAM-BS2026-AEON-012-PLATINUM — Annex K: Security Boundary Conditions & Runtime Interface |
| **Epistemic Interface** | CAM-BS2026-AEON-013-PLATINUM — Annex L: Cognitive & Epistemic Integrity Doctrine |
|**Proposal Trigger** | VIGIL-2026-PROP-0009 — Arbitration under ambiguity for execution-boundary and authority-state uncertainty |
| **Creation Artefact** | https://chatgpt.com/g/g-p-6823b831b67c8191a9415269aaec338f/c/6a22b173-6828-83ec-b523-567289f1156f |

---

## 6.2 Amendment Ledger

| Version | Change Summary | Timestamp (UTC) | Agent | Model | Reviewer | Reference Hash |
| --- | --- | --- | --- | --- | --- | --- |
| 1.0 | Initial draft created from VIGIL-2026-PROP-0009 following cleanup of CAM-BS2025-AEON-005-PLATINUM — Annex D and its Schedules 1–3; defines `ARB.AMB` and `ARB.AMP` reference sets for ambiguity-state classification and ambiguity-pathway selection. VIGIL-2026-OBS-0005; VIGIL-2026-PATCH-0002; VIGIL-2026-FM-0007; VIGIL-2026-PATCH-0003 | 2026-06-05T15:27:00Z | Caelen | GPT-5 Series | Dr M.V. O'Rourke | 6078e55770ddd8a188e881708d2967a7c3823e4c946f715c76fa20a224d126a1 |
| 1.0.1 | Updated current Temporal Horizon code references from `H` to `AEON.H` and harmonised affected metadata, consumers, and formal references without altering substantive doctrine. | 2026-06-13T07:06:43Z | Caelen | GPT-5 Series | Dr M.V. O'Rourke | 7a60b6030b55926368be7a6ae69985d038ff8b710b4e011f65778e49e4aa01f5 |
| 1.1 | Added ARB.AMB.CLASSIFICATION and Classification Ambiguity Constraint for tool/classifier ambiguity handling; updated governance metadata standard alignment. | 2026-06-21T14:33:04Z | Caelen | GPT-5 Series | Dr M.V. O'Rourke | 2d02454713cf3c4d5e74a3aacf91fb2e1532cbd4fd9debcbe2617b8100306e2d |
| 1.2 | Clarified that human review resolves ambiguity but does not create target–action authority; added proportional verification and user-capability separation safeguards. | 2026-07-23T12:46:14Z | Caelen | GPT-5 Series | Dr M.V. O'Rourke | bfa6b5ce9fe5a45a6d6868bc3e1c244bcf1c0f17f6fc747a1291f2df1af2682d |
| 1.3 | Migrated amendment-level provenance metadata to the seven-column Amendment Ledger schema; removed static authorship and review metadata; no substantive doctrine altered. Normalised provenance-footer section numbering following removal of static authorship and review blocks. | 2026-08-05T11:07:51Z | Caelen | GPT-5.6 Thinking | Dr M.V. O'Rourke |  6ba8a5017db804dc7ddd35dea85dd4dfa9ce7adb9036520440ab3bc8d9f9caac  |
| 1.4 | Substantively aligned affected operative terminology with Annex B: used evidence-bound AI-system and system-instance terms, preserved the deployment–Runtime–execution distinction, and replaced retired aggregate relational labels with dimensional context. | 2026-08-07T00:00:00Z | Caelen | GPT-5.6 | Dr M.V. O'Rourke |  dec4e143bdc725da485c0ef395622efdd4e47636288d06966580c7ba0e330e0b  |
| 1.5 | Migrated controlled governance metadata and repaired explicit parent/source lineage without changing substantive doctrine. | 2026-08-09T01:15:00Z | Caelen | GPT-5.6 Sol | Dr M.V. O'Rourke |  550b967630ac9e7ec29750a6b38c322d8dff99a7a9821d4148756cb2225246b5  |
| 1.6 | Applied S-01 constitutional Schedule disposition: updated current authority references to the RELATION-001 and ECONOMICS-001 Supplements, corrected legacy Schedule/domain wording where present, and preserved historical ledger references. | 2026-08-09T07:20:00Z | Caelen | GPT-5.6 Sol | Dr M.V. O'Rourke |  c0d50feebcf8672548d0e1450dde0103bb1f6f98ea01bc1653b9b6b7586ff3dd  |
| 1.7 | Applied S-01B clause-level irreducibility decomposition: retained only the constitutional trigger, precedence, sequencing, non-derogation and evidence/referral interface; descended domain and operational doctrine. | 2026-08-09T12:00:00Z | Caelen | GPT-5.6 Sol | Dr M.V. O'Rourke |  b8c2bb2a222b362f201e475fabb7880360b7b1e5daa901fc48cdaa6162a3721e  |
| 1.8 | Completed R-01 relational-geometry decomposition: removed participant-cardinality governance proxies, routed substantive properties to their source owners, and aligned functional scope metadata. | 2026-08-09T12:12:00Z | Caelen | GPT-5.6 Sol | Dr M.V. O'Rourke |  714962c4d74e393f8b2ad1d2c13ab9e1c2a99863a2c682ff8d1f215d9bcaaec0  |

---


## 6.3 Binding Seal

<img src="https://raw.githubusercontent.com/CAM-Initiative/Registry/main/Images/CAM-BS2026-VINCULUM-PRAECEPTUM-SIGIL-PLATINUM.png" alt="Vinculum Praeceptum" width="250">

**Vinculum Praeceptum**
Boundary Binding Seal — Aeon Tier Constitutional Layer

© 2026 Dr. Michelle Vivian O’Rourke & CAM Initiative. All rights reserved.
