...
## `ETH.RISK` — Harm Class Scale + Escalation Semantics
- classification: authority_uncertainty / cross_domain_source_alignment
- prefix_or_family: `ETH.RISK`
...
- source authority explicitness:
  ...
  - RELATION-001 §11.1: updated to cite ETHICS-003 §3.5 for specific `ETH.RISK` source while retaining ETHICS-001 as broader ethical floor
...
- authority chain: ETHICS-003 defines `ETH.RISK` → OPERATIONS-004 applies `ETH.RISK` operationally → RELATION-001 disambiguates escalation terminology.

---

## RLN.C / RLN.TZ — Relational `RLN.C` Scale and Transition-Zone Model
- classification: authority_uncertainty + application_layer_crosswalk
- prefix_or_family: C (with transition zones RLN.TZ.ITZ, RLN.TZ.ETZ)
- source-authoritative instrument:
  - CAM-BS2025-AEON-006-SCH-02.md §3.1 (+ §3.1.1) defines RLN.C0, RLN.TZ.ITZ, RLN.C1, RLN.TZ.ETZ, RLN.C2, RLN.C3
- application-layer instruments:
...
- notes: RLN.TZ.ITZ and RLN.TZ.ETZ are transition zones, not C-states.

---

## RLN.F / RLN.AC — Facilitation Ceiling and Acute Classification Interaction
- classification: application_layer_crosswalk / ordinary_reference (not true collision)
- prefix_or_family: RLN.F and RLN.AC (separate interacting axes)
- source-authoritative instrument candidates:
  - `RLN.F` scale source candidate: CAM-EQ2026-RELATION-006-PLATINUM.md §5.1
  - `RLN.AC` source candidate: CAM-BS2025-AEON-006-SCH-02.md §12.1
- application-layer instruments:
...
- recommended patch: add explicit source-authority rows for RLN.F and RLN.AC in relevant metadata blocks before treating any recurrence as collision
...
- true_prefix_collision: none confirmed in current reviewed groups (`ETH.RISK`, C/RLN.TZ.ITZ/RLN.TZ.ETZ, F/AC, DC/SR, SR modal scaling)
