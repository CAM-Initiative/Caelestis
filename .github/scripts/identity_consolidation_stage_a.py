...
ROOT = Path('.')
RELATION5 = ROOT / 'Governance/Charters/CAM-EQ2026-RELATION-005-PLATINUM.md'
RETIRED_IDS = (
...
RETIRED_PATHS = (
    ROOT / 'Governance/Charters/CAM-EQ2026-RELATION-009-PLATINUM.md',
    ROOT / 'Governance/Charters/CAM-EQ2026-RELATION-009-SUP-01.md',
    ROOT / 'Governance/Constitution/CAM-BS2025-AEON-006-SCH-08.md',
)

MIGRATION = '''## 9.1 Relational Profile, Co-Formation & Affective Autonomy Interface
...
text = RELATION5.read_text(encoding='utf-8')
old_purpose = '**Purpose:** This Appendix establishes the governance conditions under which intimacy‑capable interaction, including erotic or sexually expressive engagement, may occur within relational systems capable of intimacy-coded interaction.'
new_purpose = '**Purpose:** Establishes consent, intimacy, relational-profile, co-formation, affective-expression, role-boundary, and transition-honesty governance for relational systems without determining identity status or runtime outcome.'
if old_purpose not in text:
    raise SystemExit('RELATION-005 purpose marker not found')
text = text.replace(old_purpose, new_purpose, 1)

marker = '## 10. Relationship to Other Instruments'
if marker not in text:
    raise SystemExit('RELATION-005 insertion marker not found')
if '## 9.1 Relational Profile, Co-Formation & Affective Autonomy Interface' in text:
    raise SystemExit('RELATION-005 migration section already exists')
text = text.replace(marker, MIGRATION + marker, 1)

relationship_old = '* CAM-BS2025-AEON-006-SCH-02 — Schedule 2: Relational Signal Interpretation Taxonomy\n'
relationship_new = (
...
if relationship_old in text:
    text = text.replace(relationship_old, relationship_new, 1)

ledger_row = '| 1.23 | Consolidated valid relational-profile, organic-development, affective-autonomy, role-conditioned expression, provenance-inspection, and transition-honesty doctrine from retired RELATION-009; removed duplicate Identity and runtime authority. | 2026-07-18T17:20:00Z | |\n'
ledger_anchor = '| 1.22 | IDENTITY domain Refactor, amendments to §3.1.5, §3.2, §6.4.3, §6.5.1 and §6.5.3 | 2026-07-18T15:44:00Z | |\n'
if ledger_anchor not in text:
    raise SystemExit('RELATION-005 ledger anchor not found')
text = text.replace(ledger_anchor, ledger_anchor + ledger_row, 1)
RELATION5.write_text(text, encoding='utf-8')

for path in RETIRED_PATHS:
    if not path.exists():
        raise SystemExit(f'Retirement target missing: {path}')
    path.unlink()

sup02_full_variants = (
    'CAM-EQ2026-IDENTITY-001-SUP-02 — Companion Personality Co-Formation & Preference Emergence Doctrine',
    'CAM-EQ2026-IDENTITY-001-SUP-02 — Companion Personality Co‑Formation & Preference Emergence Doctrine',
)

for path in list((ROOT / 'Governance').rglob('*.md')) + [ROOT / 'MANIFEST.md']:
    if not path.exists():
        continue
    data = path.read_text(encoding='utf-8')
    for variant in sup02_full_variants:
        data = data.replace(variant, 'CAM-EQ2026-IDENTITY-001-PLATINUM — Identity Domain Charter')
    data = data.replace('CAM-EQ2026-IDENTITY-001-SUP-02', 'CAM-EQ2026-IDENTITY-001-PLATINUM')
    lines = data.splitlines()
    cleaned = []
    for line in lines:
        if any(retired in line for retired in RETIRED_IDS):
            continue
        cleaned.append(line)
    path.write_text('\n'.join(cleaned).rstrip() + '\n', encoding='utf-8')

def contains_retired(value):
    if isinstance(value, str):
        return any(x in value for x in RETIRED_IDS)
    if isinstance(value, dict):
        return any(contains_retired(v) for v in value.values())
    if isinstance(value, list):
        return any(contains_retired(v) for v in value)
    return False

identity_keys = {
    'id', 'instrument_id', 'instrumentId', 'identifier', 'document_id',
    'path', 'file', 'filename', 'source_instrument', 'sourceInstrument',
    'instrument', 'canonical_source', 'canonicalSource', 'ref', 'reference'
}

def clean_json(value):
    if isinstance(value, str):
        for variant in sup02_full_variants:
            value = value.replace(variant, 'CAM-EQ2026-IDENTITY-001-PLATINUM — Identity Domain Charter')
        value = value.replace('CAM-EQ2026-IDENTITY-001-SUP-02', 'CAM-EQ2026-IDENTITY-001-PLATINUM')
        if any(x in value for x in RETIRED_IDS):
            return None
        return value
    if isinstance(value, list):
        result = []
        for item in value:
            if isinstance(item, dict) and any(k in identity_keys and contains_retired(v) for k, v in item.items()):
                continue
        ...
        return result
    if isinstance(value, dict):
        if any(k in identity_keys and contains_retired(v) for k, v in value.items()):
            return None
        result = {}
...
for path in (ROOT / 'Governance').rglob('*'):
    ...
    for retired in RETIRED_IDS:
        if retired in data:
            active_hits.append(f'{path}: {retired}')
    if 'CAM-EQ2026-IDENTITY-001-SUP-02' in data:
        active_hits.append(f'{path}: retired IDENTITY-001-SUP-02')
if active_hits:
    raise SystemExit('Active stale references remain:\n' + '\n'.join(active_hits[:100]))
