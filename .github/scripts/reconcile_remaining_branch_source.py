...
def next_minor(versions):
    parsed = [semver(v) for v in versions if VERSION_RE.match(v)]
    top = max(parsed)
    if len(top) == 2:
...
def merge_ledger(path, current_ledger, old_ledger):
    ...
    # Amend malformed or placeholder-dated existing entries in place.
    for row in rows:
    ...
    working_ledger = "".join(lines)
    rows = parse_rows(working_ledger)
    additions = []
    ...
    for old_row in parse_rows(old_ledger):
        ...
        if collision:
            if is_red_team and path == "Governance/Charters/CAM-EQ2026-OPERATIONS-007-PLATINUM.md":
                version = next_minor([row["version"] for row in rows] + [item["version"] for item in additions])
            else:
                raise RuntimeError(f"Amendment version collision in {path}: {version}")
    ...
    if additions:
        current_rows = parse_rows("".join(lines))
        if not current_rows:
            raise RuntimeError(f"Cannot locate amendment table insertion point in {path}")
        insert_at = max(row["index"] for row in current_rows) + 1
        lines[insert_at:insert_at] = [
            canonical_row(item["version"], item["description"], item["timestamp"], item["hash"])
