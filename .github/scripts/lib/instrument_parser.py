...
_HELPER = _REPO_ROOT / ".github" / "scripts" / "reconcile_nonconflicting_branch_source.py"
_REPORT = _REPO_ROOT / ".github" / "Indices" / "source-reconciliation-diagnostic.txt"

if not _HELPER.exists():
    raise RuntimeError("Temporary non-conflicting reconciliation helper is missing")
...
if _proc.returncode != 0:
    ...
    _REPORT.write_text(
        "NON-CONFLICTING SOURCE RECONCILIATION FAILED\n\nSTDOUT\n======\n"
        + _proc.stdout
        + "\nSTDERR\n======\n"
        + _proc.stderr,
        encoding="utf-8",
