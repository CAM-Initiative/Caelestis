...
TARGET_DIR = REPO_ROOT / "Governance" / "Covenants"
INDEX_MD = TARGET_DIR / "CAM-Covenants-Index.md"
INDEX_JSON = TARGET_DIR / "covenants.index.json"
    ...
    for i, ln in enumerate(lines):
        if ln.startswith("#") and any(k in normalise(ln) for k in SUMMARY_KEYWORDS):
            ...
            for ln2 in lines[i + 1 :]:
                ...
                collected.append(s)
            if collected:
                summary = " ".join(re.split(r"(?<=[.!?])\s+", " ".join(collected))[:2]).strip()
...
def collect_items() -> list[dict]:
    ...
    for p in sorted(TARGET_DIR.glob("*.md")):
        ...
        parsed = parse_instrument_filename(p.name, "covenants")
        if not parsed:
            continue
